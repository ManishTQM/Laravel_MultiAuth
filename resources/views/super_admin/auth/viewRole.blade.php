...
<div><a href="{{route('showRole.index')}}"><button class="btn btn-primary my-5">Show Role</button></a></div>
            <br><table class="table table-bordered data-table user_table">
            <form action="{{url('superadmin/superAdmin_permisions/'.$role->id)}}" method="post" >
                @csrf
                <div class="form-group">
                                <label><strong>Permision :</strong></label><br>
                                @foreach($permission as $user)
                               
                                <label><input type="checkbox" name="permission[]"  value="{{$user->id}}"@foreach ($rolePermissions as $per) @if($per->permission_id == $user->id)checked @endif @endforeach>{{$user->name }}</label>
                                @endforeach
